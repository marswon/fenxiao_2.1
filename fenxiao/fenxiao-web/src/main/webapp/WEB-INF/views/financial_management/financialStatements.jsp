...
<script>
	$(function() {
		...
		$("#waitorder")
				.datagrid(
						{
							url : "${ctx}/financialManagement/getFormList",
							nowrap : false,
							striped : true,
							remoteSort : false,
							pageSize : 20,
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(val,row,index){
											if(row.businessType==1){
												if(row.flowType==1){
													var value = row.fxOrderRecord.operatorProduct.channelDiscount/1000*(row.fxOrderRecord.operatorProduct.price/1000);
	...
	function hkExportExcel() {
		$.messager.confirm('确认', '确认把该搜索结果导出Excel表格 ？', function(r) {
			if (r) {
				...
				$.ajax({
					url : "${ctx}/export/isOutOfManageFinancialExcel?startTime="
						+ startTime + "&endTime=" + endTime + "&mphone=" + mphone
					...
					success : function(data) {
						if (data.resultCode == 1) {
							location.href = "${ctx}/export/exportFinancialManageExcel?startTime="
								+ startTime + "&endTime=" + endTime + "&mphone=" + mphone
								+"&orderNo=" + orderNo + "&eId=" + eId + "&eName=" + eName
								+ "&businessType=" + businessType;						} else {
							if (data.resultMsg) {
				...
				<form id="queryHkForm">
					<table class=" datagrid-cell datagrid-cell-c1-status"
						...
						<tr>
							...
							<td>所属企业</td>
							<td><input id="eId" name="search_EQ_enterprise.id"></td>
							<td style="text-align: right; vertical-align: middle;"><span>关键词:</span></td>
							<td>
								<table>
									<tr>
										<td style="border: none; vertical-align: middle;"><input
											type="text" name="search_LIKE_fxOrderRecord.downstreamOrderNo"
											placeholder="请输入订单号" id="billCode"
											style="margin: 0; width: 180px;" /></td>
										<td style="border: none; vertical-align: middle;"><input
											type="text" name="search_LIKE_fxOrderRecord.mobile" placeholder="请输入手机号"
											style="margin: 0; width: 130px;" id="mphone" /></td>
											<td><select style="margin: 0; width: 100%;"
											name="search_EQ_businessType" id="businessType">
